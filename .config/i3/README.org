...
#+begin_src conf
exec --no-startup-id xss-lock --transfer-sleep-lock -- ~/.config/i3/scripts/locker.sh
...
#exec --no-startup-id python /home/roger/.config/i3/scripts/ws_router.py
#+end_src
...
- =$terminal= / =$auxTerminal=: Defines variables for the preferred terminal emulator (=ghostty=) and
  an auxiliary one (=alacritty=).
...
mode "$mode_power" {
    bindsym l exec --no-startup-id ~/.config/i3/scripts/locker.sh, mode "default"
...
- Screenshots: Various bindings using =scrot= and =ksnip= for capturing the full screen, a selected
  area (saving to file or clipboard), or launching the Ksnip GUI. The =--release= flag triggers the
...
- - =$mod+o=: Toggles Handy local speech-to-text transcription on/off, showing a notification on
  start/stop. The toggle script uses a state file to track session state. Handy starts hidden on login
  (see [[*Autostart Applications][Autostart Applications]]).
#+begin_src conf
...
bindsym --release Ctrl+Shift+F11 exec ksnip -r
...
# Toggles transcription on/off with a desktop notification for state changes.
# Handy itself starts hidden on login (see Autostart Applications).
bindsym $mod+o exec --no-startup-id ~/.config/i3/scripts/handy-toggle.sh
#+end_src
...
assign [class="org.mozilla.Thunderbird"] $ws5
...
exec_always --no-startup-id sh -c "autotiling || autotiling-rs"
