...
if [ -n "$CLAUDE_CODE_ENTRYPOINT" ]; then
  # Running inside Claude Code CLI — skip the local claude reviewer for independence
  SELF_CLI="claude"
else
  # Other environments — fall back to AI self-identification
  SELF_CLI="auto"
fi
```

Rules:
- If `SELF_CLI="claude"` → skip the local claude reviewer; the OpenCode (lazer/*) reviewers and Codex provide independence.
- If `SELF_CLI="auto"` → the executing AI identifies itself and skips its own CLI.
- The OpenCode reviewer set (Gemini, MiniMax, Kimi, GLM-5, Qwen, DeepSeek) is always independent of the host Claude session.
- At least one reviewer DIFFERENT from the host must be available for the review to proceed. If only `claude` is available and we are already running inside Claude, at least one non-claude reviewer CLI (`codex` or `opencode`) must also be available to ensure independence.
