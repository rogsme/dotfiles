...
**Step B: Proceed to update_roadmap.** Do NOT block for user approval. The UAT file persists with `status: partial` and will surface in `/gsd-progress` and `/gsd-audit-uat`. The user runs `/gsd-verify-work` when ready.

Display:
```
## ✓ Phase {X}: {Name} — Human Verification Pending

All automated checks passed. {N} items need manual testing.
Items saved to `{phase_num}-HUMAN-UAT.md`.
```
...
/gsd-code-review {X} ${GSD_WS} — review phase code quality  ← recommended
/gsd-verify-work {X} ${GSD_WS} — manual UAT testing
