...
@/home/roger/.config/opencode/gsd-core/references/agent-contracts.md
@/home/roger/.config/opencode/gsd-core/references/context-budget.md
@/home/roger/.config/opencode/gsd-core/references/gates.md
</required_reading>
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/home/roger/.config/opencode/gsd-core/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/home/roger/.config/opencode/gsd-core/references/planner-antipatterns.md
...
```
Pure doc-only / config-only / test-only tasks return `is_behavior_adding=false` and are exempt. When the gate trips, Read `/home/roger/.config/opencode/gsd-core/references/execute-mvp-tdd.md` for the exact halt report format.
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/home/roger/.config/opencode/gsd-core/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       <execution_context>
       @/home/roger/.config/opencode/gsd-core/workflows/execute-plan.md
       @/home/roger/.config/opencode/gsd-core/templates/summary.md
       @/home/roger/.config/opencode/gsd-core/references/checkpoints.md
       @/home/roger/.config/opencode/gsd-core/references/tdd.md
       @/home/roger/.config/opencode/gsd-core/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/home/roger/.config/opencode/gsd-core/references/executor-examples.md'}
       </execution_context>
...
**LOCAL PATCH — no mandatory inline code review.** This step does NOT auto-invoke a code-review subagent or skill inline. Code review is a *suggested next action only* (surfaced in `offer_next`), never a mandatory blocking gate. Skip directly to the next step.

`/gsd-code-review` remains available for the user to run on their own when they want a phase code-quality pass — it is offered in the completion routing, not enforced here.

ALWAYS proceed to close_parent_artifacts → regression_gate → verify_phase_goal.
</step>
...
Route on `$STATUS`: if `passed`, proceed to update_roadmap. **LOCAL PATCH:** if `human_needed`, do NOT keep the phase pending — treat it as a non-blocking completion and follow the **If human_needed** branch below (proceed to update_roadmap, no UAT artifact, no wait). For any other non-`passed` status (e.g. `gaps_found`, `missing`, `unknown`), keep the phase pending — present `$NEXT_ACTION` to the user and, when `$NEXT_COMMAND` is non-empty, show it as the next command to run.

**If human_needed:**

**LOCAL PATCH — human_needed is non-blocking and does NOT create UAT artifacts.** Do NOT create or commit a `{phase_num}-UAT.md` file — building UAT artifacts is `/gsd-verify-work`'s responsibility, not execute-phase's. Do NOT block phase completion waiting for manual testing.

Proceed to update_roadmap. The human verification items remain listed in `{phase_dir}/{phase_num}-VERIFICATION.md` and will surface in `/gsd-progress` and `/gsd-audit-uat`.

Display:
```
## ✓ Phase {X}: {Name} — Human Verification Items

All automated checks passed. {N} item(s) need manual testing (listed in VERIFICATION.md).
Run `/gsd-verify-work {X} ${GSD_WS}` when ready to test them — it is a suggested next action, not a blocking gate.
```

Then continue to update_roadmap. Do NOT wait for user approval.
...
Read and follow `/home/roger/.config/opencode/gsd-core/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
...
/gsd-code-review {X} ${GSD_WS} — review phase code quality (suggested, optional)
/gsd-verify-work {X} ${GSD_WS} — manual UAT testing (suggested, optional)
