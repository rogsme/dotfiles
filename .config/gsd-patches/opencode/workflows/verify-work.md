...
<template>
@/home/roger/.config/opencode/gsd-core/templates/UAT.md
</template>
...
**Text mode (`workflow.text_mode: true` in config or `--text` flag):** Set `TEXT_MODE=true` if `--text` is present in `$ARGUMENTS` OR `text_mode` from init JSON is `true`. When TEXT_MODE is active, replace every `question` call with a plain-text numbered list and ask the user to type their choice number. This is required for non-the agent runtimes (OpenAI Codex, Gemini CLI, etc.) where `question` is not available. This applies to every `question` call in this workflow, including the `--auto` auto-verify prompts below.
</step>
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@/home/roger/.config/opencode/gsd-core/references/verify-mvp-mode.md`. Briefly:
...
Read and follow `/home/roger/.config/opencode/gsd-core/workflows/transition.md`.
...
- Follow @/home/roger/.config/opencode/gsd-core/workflows/diagnose-issues.md
