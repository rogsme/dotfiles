...
function =make-llm-lazer-compatible= to work correctly. Wrapping it in a helper allows for easy reuse
and potential lazy-loading in the future.
...
:PROPERTIES:
:ID:       29bfae86-f79d-4965-b179-80c5a930cb00
:END:

This helper wraps =make-llm-openai-compatible= from the =llm= package and points it to my Lazer endpoint.
This keeps the provider switcher clean: it just passes the model name, and the wrapper handles URL and
auth.
...
Lazer exposes a single OpenAI-compatible endpoint where I can access all the models I use for commits,
PR descriptions, and general coding tasks.
...
- =SPC l f=: Calls =my/set-forge-llm-provider= to switch the active provider for Forge LLM. Useful when I
  want to use a different model (e.g. GLM-5 vs MiniMax M2.5) for generating pull request descriptions.
...
This config block sets up =chatgpt-shell=, a shell-like interface to interact with Lazer's
OpenAI-compatible endpoint directly inside Emacs. It’s great for general-purpose LLM queries, quick code
...
- API endpoint and key: =chatgpt-shell= uses my Lazer base URL and API key from =private.el=.

This setup gives me flexibility to work with multiple models while maintaining a consistent shell-style
UX inside Emacs.
...
well-written Git commit messages based on the current file diffs.

To make switching models easier, I also added =my/set-magit-gptcommit-provider=, an interactive command
that lets me choose the active LLM provider from my Lazer model catalog on the fly. This is super handy
when I want to test different models' output depending on the context.
...
This section sets up Forge LLM to work with my Lazer model catalog, allowing me to dynamically choose
which model to use with ~M-x my/set-forge-llm-provider~ or ~SPC l f~.

The default provider is set to GLM-5, but I can call ~my/set-forge-llm-provider~ at any time to switch
providers on the fly. The selected LLM is then used by Forge LLM when I run pull request generation.
...
- =aidermacs-default-model=: Sets Qwen3 Coder 480B Turbo as the default model, but this can be switched
  easily.
