...
Both of these helpers wrap =make-llm-openai-compatible= from the =llm= package, each pointing at a
different backend. This keeps the provider switcher clean: it just calls the right wrapper with a model
name, and the wrapper handles URL and auth.
...
OpenRouter's unified endpoint gives access to models from multiple providers (Qwen, Gemini, etc.) with a
single API key. I use this for models that aren't covered by my Claude Max subscription.
...
**** CLIProxyAPI
:PROPERTIES:
:ID:       2703b0bf-e6c2-48d3-b3e7-fd166fe2f76d
:END:

[[https://github.com/router-for-me/CLIProxyAPI][CLIProxyAPI]] exposes my Claude Max subscription as an OpenAI-compatible API on =localhost:8317=. This
means I can use Claude models (Haiku, Sonnet, Opus) through the =llm= package without paying for
separate API access. No API key needed since the proxy runs locally and handles OAuth authentication
with my subscription. See [[https://rogs.me/2026/02/use-your-claude-max-subscription-as-an-api-with-cliproxyapi/][my blog post]] for the full setup.
...
that lets me choose the active LLM provider (Gemini, Claude, Qwen, or other) on the fly. This is super
